...
-- invoice_app_users no longer needs an auth.users account behind it (no signup
-- flow exists) — decouple so Admin can create users freely.
alter table invoice_app_users drop constraint invoice_app_users_id_fkey;
alter table invoice_app_users alter column id set default gen_random_uuid();
...
-- change capture (T150).
alter table invoice_audit_events add column invoice_id uuid references invoices(id);
alter table invoice_audit_events add column changes jsonb;
