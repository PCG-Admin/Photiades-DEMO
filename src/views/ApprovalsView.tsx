...
export function ApprovalsView({ initialItems }: { initialItems: ApprovalInboxItem[] }) {
  ...
  return (
    <div className="view-enter">
      <PageHeader title="Approvals" sub="Stock and Non-Stock invoice workflow tasks awaiting a decision."
        actions={<button className="btn" onClick={() => go('workflows')}><I.zap size={15} />Open Workflows</button>} />

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 'var(--gap-4)', marginBottom: 'var(--gap-5)' }}>
        <MiniStat label="Awaiting a decision" value={items.length} sub="across both workflows" tone="blue" />
        <MiniStat label="Total value pending" value={fmtMoney(items.reduce((s, i) => s + i.amount, 0))} tone="violet" />
        <MiniStat label="High priority" value={items.filter(i => priorityOf(i.amount) === 'High').length} sub="> €50,000" tone="red" />
      </div>
              ...
              return (
                <button key={item.instance.id} onClick={() => setSelCode(item.instance.code)}
                  ...
                  <div className="row" style={{ justifyContent: 'space-between', gap: 8 }}>
                    ...
                    <Badge tone={prioTone[priority]} dot>{priority}</Badge>
                  </div>
            ...
            <div className="card-head">
              <div>
                <div className="row" style={{ gap: 9 }}>
                  ...
                  <Badge tone={{ High: 'red', Medium: 'amber', Low: 'gray' }[priorityOf(sel.amount)]} dot>{priorityOf(sel.amount)} priority</Badge>
                </div>
                <div className="card-title" style={{ marginTop: 6 }}>{sel.vendor} — {sel.currentTaskName}</div>
              </div>
              <button className="btn ghost sm" onClick={() => go('invoices', sel.invoiceCode)}>View invoice<I.arrowR size={14} /></button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: 'var(--gap-5)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14, marginBottom: 24 }}>
                <DetailField label="Amount" value={fmtMoney(sel.amount)} big />
                <DetailField label="Invoice" value={sel.invoiceCode} mono />
                <DetailField label="Current task" value={sel.currentTaskName} />
                <DetailField label="Assigned role" value={sel.instance.assignee_role} />
                <DetailField label="Started" value={<RelativeTime date={new Date(sel.instance.started_at)} />} />
                <DetailField label="Workflow" value={sel.instance.wf_id === 'stock' ? 'Stock' : 'Non-Stock'} />
              </div>
              <div className="card" style={{ padding: 14, background: 'var(--surface-2)', fontSize: 13, lineHeight: 1.6, color: 'var(--text-2)' }}>
                Awaiting <strong>{sel.currentTaskName}</strong> — open Workflows to review the full task history and submit a decision.
              </div>
            </div>
            <div style={{ padding: 'var(--gap-4) var(--gap-5)', borderTop: '1px solid var(--border)' }}>
              <button className="btn primary" style={{ width: '100%' }} onClick={() => go('workflows')}><I.zap size={15} />Open in Workflows</button>
            </div>
          </div>
        ) : (
